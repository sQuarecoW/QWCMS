...
//            
//            return EventLoopFuture.map(on: tag) { .string("<!-- included \(asset.name) from \(tag.source.file.split(separator: "/").last ?? "unknown"): \(tag.source.line) -->") }
//        }
//        
//        return EventLoopFuture.map(on: tag.container) { .string("<!-- asset \"\(asset)\", included from \(tag.source.file): \(tag.source.line) not available -->") }
//    }
//}
